...
1. **Estimators are definitions.** Where a paper displays an estimator, the library records a
   ...
   `bbmwEstimation`, `rohrMsep`, `rohrHorizonRelMsepApprox`, `msepW`, `glmCellMsep`,
   `glmAggregateMsep` and the
...
5. **CI audit.** `VerifiedReserving/Test/Axioms.lean` runs `#print axioms` on 479 named theorem and
   witness declarations. CI rejects a nonstandard axiom, rejects any `sorry`, `admit` or custom
   `axiom` declaration, and runs the core Mack witness. The generated blueprint manifest contains
   660 links to 659 distinct declarations, all validated against the library.
...
| Quarg-Mack 2004, Sections 2.2 and 3.1.2: paid/incurred residual regressions and adjusted recursion | `MunichRowsIndep`, `MunichPaidRegression`, `MunichIncurredRegression`, `munichPaid_residualCorrelation_eq`, `munichIncurred_residualCorrelation_eq`, `munichProjection` | PIU is stated for the finite `n` accident years and their histories through `n - 1`; PQ and IQ are exact assumptions for adjacent periods `k + 1 < n`; the correlation results state integrability, measurability and standardization explicitly; the practical estimators and recursion are definitions |
...
`jointRowSigma` joins their row histories. `MunichRowsIndep` states PIU for the finite `n` accident
years using joint histories through the final period `n - 1`. `conditionalResidual` uses the
conditional mean and the square root of the conditional variance. `MunichPaidRegression` and
`MunichIncurredRegression` state PQ and IQ directly as conditional-expectation equalities for
adjacent periods satisfying `k + 1 < n`. The generic theorem
`condExp_mul_eq_slope_of_residual_regression` assumes `𝒜 ≤ ℬ`, integrability of `Z` and `A * Z`,
...
conditional expectations given `D_d`, and the
...
cells with `i < n`, `j < n` and `i+j ≤ t`; `calendarSigma_mono` proves it is a filtration.
For `i < n` and `k < n`, `calendarSigma_eq_sup` proves that at time `i+k` it is `rowSigma i k` joined with
the available cells from all other rows, and `condExp_calendarSigma_eq_rowSigma` removes the independent
enlargement. `RandomTriangle.calendarTrueCDR X μ i t` is the difference of the two conditional
expectations. `condExp_calendarTrueCDR_eq_zero` assumes only a finite measure; as in the
development-year result, the equality has its intended probabilistic meaning when the ultimate
is integrable because conditional expectation is set to zero off the integrable case.
...
formalized is the marginal-sum system as reported in the cited secondary literature, citing also England and Verrall,
...
`integral_buhlmannStraubPredictor_eq_collectiveMean` proves, when `P ≠ 0`, that every scalar
predictor `m+z(Xbar-m)` is expectation-unbiased, matching the constraint in the source's Section 6.
...
term. Under the same `P ≠ 0` condition, for every scalar `z`,
`buhlmannStraubExpectedSqLoss_eq` then proves
...
## Bornhuetter-Ferguson prediction error
...
mean `x_i(1-z_d)` and variance `x_i ∑_{d≤k<m} σ_k²` for `d≤m`, under integrability or square integrability.
...
source's best-linear-unbiased claim when `σ_k²≥0`. `bfResidualSumSqRv_eq` gives the residual sum-of-squares
