...
def main():
    ...
    findings = open(findings_path).read() if os.path.exists(findings_path) else "(FINDINGS.md not present)\n"
    with open(os.path.join(HERE, "AUDIT_RESULTS.md"), "w") as fh:
        ...
        fh.write("Generated by `audit.py` (one command: `./run.sh`). Certificates per company are in `certificates/<line>_<GRCODE>.json`; "
