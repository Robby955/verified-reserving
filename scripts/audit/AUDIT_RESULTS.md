...
Generated by `audit.py` (one command: `./run.sh`). Certificates per company are in `certificates/<line>_<GRCODE>.json`; machine-readable aggregates in `summary.json`; LaTeX table bodies in `tables.tex`; sources and hashes in `SOURCES.md`.
