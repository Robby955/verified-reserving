...
General weights and exponent (`VerifiedReserving/Mack1999.lean`). Mack's 1999 paper also widens the factor estimator to `f̂_k = ∑_i w_ik C_ik^α F_ik / ∑_i w_ik C_ik^α`, with weights `w_ik` and an exponent α; α = 1 with unit weights is the 1993 estimator, α = 0 the simple average of the individual factors, α = 2 the regression through the origin. The whole family is defined here (`fhatW`, `sigma2W`, the projection `ChatW`, the closed form `msepW` which is the paper's equation (*), and the recursion `se2recW` displayed below it), and `se2recW_eq_msepW` proves the recursion returns the closed form for every weight function and every exponent, under the same nonvanishing hypothesis on the development factors as the unit-weight case. `fhatW_eq_weighted_average` and `weighted_sq_devW_at_fhatW` are the weighted-mean and sum-of-squares identities of the deterministic layer in this generality; `msepW_unit`, `se2recW_unit` and `se2recW_eq_msep_unit` collapse everything back to Mack 1993 at α = 1 with unit weights, so the general theorem contains `se2rec_eq_msep`. In the stochastic layer `condExp_fhatWrv` proves the weighted estimator is conditionally unbiased for predictable weights. `Mack3W` states the `D_k`-conditioned form of the paper's weighted second-moment assumption, `condExp_sq_fhatWrv_sub` proves `E[(f̂_k-f_k)² | D_k] = σ_k² / ∑_j w_jk C_jk^α`, and `condVar_fhatWrv` identifies this with the conditional variance under conditional unbiasedness. Under the same weighted and cross-year assumptions, `k+3≤n`, predictable weights, nonzero contributing claims and weighted volumes, and the stated integrability hypotheses, `condExp_sigma2Wrv` proves that `sigma2W` is conditionally unbiased. Zero-weight omissions are outside that theorem because the current estimator retains Mack's fixed `n-k-2` denominator; they require an active-contributor set and its corresponding degrees of freedom. The tail factor of Section 3 is a convention whose parameters the actuary supplies, so `tailUltimate` and `tailSe2Step` are definitions and no theorem uses them.

The one-year claims development result (`VerifiedReserving/CDR.lean`, `VerifiedReserving/CDRCalendar.lean`). The development-year form and the source's calendar-year form are both present. `calendarSigma t` is generated by the cells with `i+j≤t`; at time `i+k` it splits into row `i` through `k` plus independent information from the other rows. The true calendar CDR has conditional mean zero, equals `(∏_{j>k} f_j)(f_k C_{i,k} - C_{i,k+1})`, and has conditional second moment `(∏_{j>k} f_j)² σ_k² C_{i,k}` under (M1row), (M3row) and accident-year independence. The observable CDR remains a definition because its distribution uses an approximation.
...
Two hundred and sixty-five declarations are audited as of the v0.1.0 release. The branch-local count is updated only when this work is integrated. The remaining bounded extensions are recorded in [ROADMAP.md](ROADMAP.md).
...
The first versioned release is [v0.1.0](https://github.com/Robby955/verified-reserving/releases/tag/v0.1.0). [CHANGELOG.md](CHANGELOG.md) records its contents, and [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) records the release checks.
