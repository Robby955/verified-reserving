...
Mack, \emph{The Prediction Error of Bornhuetter/Ferguson} \cite{Mack2008BF}, states the
...
\[
 E[R_i]=x_i\sum_{d\leq k<m}y_k.
\]
If also $d\leq m$, this equals $x_i(1-z_d)$.
...
\]
For $i<n$ under BF1, BF3, the stated square-integrability assumptions, and
independence and equal means of the BF estimate and true future reserve,
\[
 \operatorname{msep}(\widehat R^{BF})
 =\operatorname{Var}(\widehat R^{BF})
  +x_i\sum_{d\leq k<m}\sigma_k^2.
\]
Under common independence from an observed-data sigma-algebra, square integrability,
and measurability of the prediction-target pair, the conditional expectation of
the squared prediction error equals that same constant almost surely.
\end{theorem}
...
has variance $\sigma_k^2/\sum_i x_i$. If $\sigma_k^2\geq0$, every linear unbiased competitor,
