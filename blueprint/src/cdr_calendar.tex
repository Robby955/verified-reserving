...
\uses{def:calendarSigma, def:rowAssumptions, thm:condExp_sup}
For $i<n$ and $k<n$, at time $i+k$ the calendar triangle is row $i$ through development
$k$ joined with the observed parts of the other rows. Under accident-year independence,
...
The raw mean-zero equality needs only a finite measure; since conditional expectation is
zero off the integrable case in the formalization, it has its intended probabilistic meaning
when the ultimate is integrable. For $i<n$ and $k<n-1$, under the row-conditioned Mack assumptions, independent accident
years, row integrability, and squared-residual integrability, at time $i+k$ it equals
\[
