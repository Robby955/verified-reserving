...
\begin{theorem}[Uncorrelated development factors]\label{thm:uncorrelated}
\uses{thm:unbiased}
Under (M1), for $j < k$, $E[\hat f_j \hat f_k \mid \mathcal{D}_j] = \hat f_j f_k$, so $\hat f_j$ and
$\hat f_k$ are uncorrelated (Mack 1993, Theorem 2, second part).
\end{theorem}
\begin{proof}
Tower property with $\mathcal{D}_k \supseteq \mathcal{D}_j$ and Theorem~\ref{thm:unbiased}.
\end{proof}
...
\uses{def:Mack3, thm:unbiased, lem:sq_fhat}
...
\uses{def:msep, thm:unbiased, thm:uncorrelated, thm:sigma2}
